...
We start by considering the type of mathematical programming problems we are interested in solving. Let $x \in \reals^n \times \braces{0,1}^p$, to which we will refer to as \emph{first-stage} variables. Also, most of the time, we will assume that a set of \emph{scenarios} $s \in S$ is vailable.
...
%The sets $X$ and $Y_s$, $\forall s \in S$, define bounds and nonnegativity conditions of the continuous variables.
...
\begin{align}
	...
	\st   & Ax = b \\
		  ...
		  & y^2_{s^2} \in Y^2_{s^2}, \ \forall s^2 \in S^2(s^1), s^1 \in S^1. \\
\end{align}
...
%
\begin{align*}
	\mathcal{Q}(x) = \mini_y & \sum_s p_s q_s^\top y_s \\	
...
\end{equation*}
%
Assume that we have an \emph{oracle} that can solve $\mathcal{Q}(x)$ for any given $x$ and returns an optimal solution $(y^*, u^*)$ if it exists. The CCG method can be summarised as follows.
...
For example, for a fixed $u \in \mathcal{U}$, assuming that $y \in \reals^m$, we can use strong duality to reformulate the inner problem, obtaining
%
\begin{align*}
...
\begin{enumerate}
	\item The CCG method can be seen as a primal equivalent of the Benders decomposition. In fact, one can use the same idea of generating primal variables and constraints to the main problem in the context of the L-Shaped method. \citet{GAMBOA2021} show a comparison between variants of the L-Shaped method and CCG for a variant of the 2SSP problem that incorporates distributionally robustness requirements.
