...
Since we are using linear programming duality, we must require that all second-stage variables $y_s$, $\forall s \in S$, are continuous (i.e., $q = 0$ and $Y = \braces{y_s \geq 0, \forall s \in S}$). This is an assumption that can be relaxed using an alternative variant of the method \citep{LAPORTE1993}, but we will leave this discussion out for now. 
