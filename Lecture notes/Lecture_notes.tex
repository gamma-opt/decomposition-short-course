...
Specifically, nonanticipativity constraints can have several forms. For examples, they can have a
%
\begin{enumerate}
...
\end{align*}
%
where $g_s = x_{\overline{s}} - x_s$, $\forall s \in S \setminus \{\overline{s}\}$ and $\overline{s}$ is a reference scenario. Notice that the Lagrangian function is completely separable scenario-wise. To see that, we can redefine it as
%
\begin{equation*}
...
The Lagrangian dual problem is, in essence, a non-differentiable bi-level problem and, thus, requires specific solution techniques. For linear 2SSP models following our representation, the Lagrangian dual problem is a piecewise linear concave function. This means that a nonsmooth optimisation method must be employed to solve it.

\subsection{The subgradient method}

The subgradient method is based on the idea of successively taking ascending subgradient steps in the space of the Lagrangian dual variables. The subgradient is a generalisation of the gradient for non-differentiable functions. In our particular case, $g_s$, $\forall s \in S$, is a subgradient for $L_s(\lambda)$. As we would like to maximise $L(\lambda)$, we can take steps of the form
%
\begin{equation*}
...
\end{equation*}
%
where $\alpha^k \in (0, 2]$ and is such that $\alpha_k \rightarrow 0$ as $k \rightarrow \infty$ and $\sum_k alpha_k \rightarrow \infty$ as $k \rightarrow \infty$; $\overline{z}$ is an upper bound on the optimal solution of the 2SSP $z^*$ and $L(\lambda^k)$ is the value of the Lagrangian dual function, evaluated at iteration $k$. 
...
{\bf Remark:} in general, for a Lagrangian dual function $\phi(\lambda)$, the cutting plane representing a local linearisation is of the form
\begin{equation*}
...
One drawback of the cutting-plane method is the fact that in the early iterations, we rely on a poor approximation of the Lagrangian function, which in turn may lead to poor Lagrangian dual multipliers. This vicious cycle tends to last for a while in practice and may compromise the performance of the method.
...
More precisely, at iteration $k$ of the bundle method, the Lagrangian dual multipliers are updated by solving the Lagrangian dual subproblem:
%
\begin{align*}
	\maxi_{\theta, \lambda} & \theta - \alpha ||\lambda - \overline{\lambda}^k||_2^2\\
	\st & \theta \leq \sum_s \left[p_s (c^\top x_s^l + q_s^\top y_s^l)+  \lambda_s^\top g_s^l \right], \ l = 1, \dots, k,			
\end{align*}
%
where $\overline{\lambda}^k$ represents the centre of gravity at iteration $k$ and $\alpha$ is a penalisation for deviating from $\overline{\lambda}^k$. Now, to understand the logic behind the serious step update, let us first discuss how we can assess the quality of our working representation of the Lagrangian dual function, which, at iteration $K$ is composed by a \emph{bundle} of hyperplanes of the form
%
\begin{equation*}
	\theta \leq \sum_s \left[p_s (c^\top x_s^l + q_s^\top y_s^l)+  \lambda_s^\top g_s^l \right], \ l = 1, \dots, k
\end{equation*}
%
and for which $(\theta^{k}, \lambda^{k})$ represents an optimal solution. Notice that $\theta^k$ represents the value of the Lagrangian dual function approximation at $\lambda^k$, our current \emph{trial point}. A trial point will be allowed to become the current centre of gravity if the difference between the value of the Lagrangian dual function approximation at the trial point $\lambda^k$ and the Lagrangian dual function evaluated at the centre of gravity $\overline{\lambda}^k$, $L(\overline{\lambda}^k)$, is within a fraction of the difference between the Lagrangian dual function evaluated both at $\lambda^k$, $L(\lambda^k)$, and $L(\overline{\lambda}^k)$.

More precisely, we know that the following holds in general:
%
\begin{equation*}
	\theta - L(\overline{\lambda}^k) \geq L(\lambda^k) -  L(\overline{\lambda}^k),
\end{equation*}
%
as $\theta \geq L(\lambda^k)$, recalling that if we had all $K$ cuts generated, they would otherwise match. Therefore, if we consider the ratio
%
\begin{equation*}
	\frac{L(\lambda^k) -  L(\overline{\lambda}^k)}{\theta - L(\overline{\lambda}^k)},
\end{equation*}
%
we can have an indication of how precise the approximation currently is since the closer this ratio is to one, the closer the values $\theta^k$ and $L(\lambda^k)$ are. 

The serious step rule consists of checking at each iteration whether the fraction is at least a prespecified value $m$, and if so, the centre of gravity is updated with the current iterate $\lambda^k$. Specifically,
\begin{equation*}
	\overline{\lambda}^{k+1} = 
	\begin{cases}
		\lambda^{k+1}, &\text{ if } \frac{L(\lambda^k) -  L(\overline{\lambda}^k)}{\theta - L(\overline{\lambda}^k)} \geq m \\
		\overline{\lambda}^{k}, &\text{ otherwise. }
	\end{cases}
\end{equation*}

{\bf Remark:} Bundle methods are known for being sensitive to the parametrisation of $\alpha$ and $m$, but are known for having a more robust practical convergence in the context of Lagrangian decomposition. Moreover, the method can be employed in other contexts as well as a means to solve more general (e.g., convex nonlinear) constrained optimisation problems.
...
\begin{enumerate}
	...
	\begin{align*}
		L(\lambda^k) = \mini & \sum_s p_s (c^\top x_s + q_s ^\top y_s) + \sum_s \lambda^k_s g_s \\
			\st & Ax_s = b, \ \forall s \in S \\
			& T_sx_s + W_sy_s = h_s, \ \forall s ín S \\
			& x_s \in X, \ \forall s \in S \\
...
Progressive hedging (PH) \citep{ROCKAFELLAR1991} is a method also based on Lagrangian decomposition. However, it relies on additional features to allow for complete separation while recovering the differentiability of the Lagrangian dual function.
