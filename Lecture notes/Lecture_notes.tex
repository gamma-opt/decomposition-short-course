...
\end{equation*}
%
and for which $(\theta^{k}, \lambda^{k})$ represents an optimal solution. A trial point will be allowed to become the current centre of gravity if the difference between the value of the penalised Lagrangian dual function approximation at the \emph{trial point} $\lambda^k$ and the Lagrangian dual function evaluated at the centre of gravity $\overline{\lambda}^k$, $L(\overline{\lambda}^k)$, is within a fraction of the difference between the Lagrangian dual function evaluated at $\lambda^k$, $L(\lambda^k)$, and $L(\overline{\lambda}^k)$.

More precisely, the centre of gravity $\overline{\lambda}^k$ is updated if
%
\begin{equation*}
	\frac{M^k - L(\overline{\lambda}^k)}{L(\lambda^k) -  L(\overline{\lambda}^k)} \geq m.
\end{equation*}
%
This ratio can be understood as requiring that the ascent predicted in the current iteration using the penalised Lagrangian dual subproblem, represented by $M^k - L(\overline{\lambda}^k)$ is at least a fraction of the ascent observed when using the actual Lagrangian dual function, given by $L(\lambda^k) - L(\overline{\lambda}^k)$. The serious step rule consists of checking at each iteration whether this ratio is at least a prespecified value $m$, and if so, the centre of gravity is updated with the current iterate $\lambda^k$. Specifically,
\begin{equation*}
	\overline{\lambda}^{k+1} = 
	\begin{cases}
